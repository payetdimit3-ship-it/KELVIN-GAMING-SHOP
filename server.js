...
function getUserByToken(req) {
  ...
  const email = sessions[cleanToken];
...
app.post('/api/auth/logout', (req, res) => {
  ...
  if (token) { const sessions = readJson(sessionsFile, {}); delete sessions[token.replace('Bearer ', '').trim()]; writeJson(sessionsFile, sessions); }
...
function normalizeAzamProvider(network, phone) {
  ...
  if (p) {
    ...
    if (['68', '69', '78', '79'].includes(prefix)) return 'Airtel';
...
app.post('/api/azampay/pay', async (req, res) => {
  try {
    ...
    const { amount, phone, network, items } = req.body;
    if (!amount || Number(amount) < 100) return res.status(400).json({ error: 'Kiasi cha malipo si sahihi.' });
    ...
    logSecurity('AZAMPAY_INITIATED', 'Ombi la AzamPay limetumwa kwa ' + phoneFull, 'LOW', getIP(req));
    res.json({
      success: true,
      tx_ref,
      status: 'pending',
...
app.post('/api/azampay/callback', (req, res) => {
  try {
    const body = req.body || {};
    const externalReference = body.externalreference || body.externalReference || body.externalId;
    const transactionStatus = String(body.transactionstatus || body.transactionStatus || body.status || '').toLowerCase();

    if (!externalReference) return res.status(400).json({ success: false, error: 'External Reference missing' });

    const orders = readJson('orders.json', []);
    const order = orders.find(o => o.tx_ref === externalReference);

    if (order && order.status !== 'successful') {
      if (['successful', 'success', 'completed', 'paid'].includes(transactionStatus)) {
        order.status = 'successful';
        order.confirmedAt = new Date().toISOString();
        order.azampayTxId = body.transid || body.transactionId || null;
        writeJson('orders.json', orders);
        logSecurity('AZAMPAY_SUCCESS', 'Malipo ya AzamPay yamekamilika: ' + externalReference, 'LOW', 'AzamPay-Server');
      } else if (['failed', 'cancelled', 'rejected'].includes(transactionStatus)) {
        order.status = 'failed';
        writeJson('orders.json', orders);
      }
    }
...
app.post('/api/ai/chat', async (req, res) => {
  ...
  const prompt = 'Wewe ni msaidizi wa duka la gaming la Tanzania liitwalo GameHub.\n' +
    ...
    'Malipo: AzamPay (M-Pesa, Tigo Pesa, Airtel Money, HaloPesa, Azampesa), au malipo ya moja kwa moja (manual) kwa namba tuliyotoa kwenye checkout.\n' +
...
// Server inaanza — rudisha data kutoka Supabase KWANZA
restoreFromSupabase()
